...
def eval_classifier():
    """
    Evaluate classifiers on a held-out 20% test split (fixed seed for reproducibility).
    CAVEAT: CLASSIFIER_DATASET was hand-crafted alongside the classifier keyword list,
    so even a held-out split only partially removes circular evaluation bias.
    A fully unbiased evaluation requires an independent corpus (e.g. real emergency
    radio transcripts) not seen during vocabulary design.
    """
    import random as _rng
    labels = ["NORMAL", "MEDIUM", "HIGH"]
    lbl_idx = {l: i for i, l in enumerate(labels)}

    # Reproducible 80/20 split — same split every run
    shuffled = list(CLASSIFIER_DATASET)
    _rng.Random(0).shuffle(shuffled)
    split    = int(0.8 * len(shuffled))
    test_set = shuffled[split:]       # held-out 20%

    results = {}
    for tag, fn in [("v1", classify), ("v2", classify_v2)]:
        conf = [[0]*3 for _ in range(3)]
        for text, true_lbl in test_set:
        ...
        results[tag] = {
            ...
            "confusion":    conf,
            "class_labels": labels,
            "n_test":       total,   # expose test-set size for transparency
        }
    return results
...
def generate_traffic_sequence(ticks, rate, emerg_frac, scenario, n):
    ...
    for t in range(ticks):
        ...
        if scenario == "ramp":
            ...
            # NOTE: disaster always pushes emergency fraction high, which specifically
            # activates every adaptive-mode advantage (preemptive eviction, emerg_mult).
            # The user's emerg_frac slider is respected as a floor so the slider
            # still matters; 40% is the minimum to qualify as a mass-casualty event.
            eff_emerg = max(emerg_frac, 40)
        elif scenario == "voice":
...
# Adaptive mode: full priority + congestion + queue + delay awareness.
W_ADAPTIVE = {"cong": 1.2, "queue": 0.9, "delay": 0.6, "loss": 0.7, "prog": 0.8, "emerg_mult": 2.5}

# Baseline mode: queue-aware greedy (ECMP-style).
# Previous version had all weights at zero (pure geographic greedy) — a strawman
# that no real protocol resembles. This version avoids overloaded queues and
# congested nodes while still preferring geographic progress, matching
# a basic QoS-aware shortest-path router. Comparisons against this baseline
# are conservative and more credible.
W_BASELINE = {"cong": 0.3, "queue": 0.5, "delay": 0.0, "loss": 0.0, "prog": 1.0, "emerg_mult": 1.0}
...
def sim_run(topo, traffic_seq, mode, proc_cap):
    ...
    def deliver(pkt, t):
        # Latency in simulation units — see TICK_MS / HOP_MS note above.
        lat = (t - pkt["born"]) * TICK_MS + pkt["hops"] * HOP_MS
...
def aggregate_stats(runs):
    ...
    return {
        ...
        # Per-run scalar lists — used by welch_pvalue() in the results table.
        # Without these, significance testing collapses to a heuristic.
        "runs_emerg_lat":   em,
        "runs_med_lat":     md,
        "runs_normal_lat":  nm,
        "runs_p95_emerg":   p95e,
...
class SimWorker(QThread):
    ...
    def run(self):
        ...
        # Use prime-spaced seeds so consecutive runs sample genuinely different
        # regions of the random-state space. Seeds 42, 43, ..., 71 (consecutive)
        # produce correlated topologies; step by a prime avoids this.
        _SEED_STEP = 97
        for run_i in range(num_runs):
            run_seed = seed + run_i * _SEED_STEP
...
class ResultsPanel(QWidget):
    ...
    def render_results(self, R):
        ...
        def _build_table(rows_data):
            ...
            t.setColumnCount(5)
            t.setHorizontalHeaderLabels(["Metric", "Adaptive (AI Agent)", "Baseline (QoS-Aware)", "Δ Change", "p-value"])
            t.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
            t.setEditTriggers(QTableWidget.NoEditTriggers)
            t.setAlternatingRowColors(False); t.verticalHeader().setVisible(False)
            t.setRowCount(len(rows_data))
            for ri, (metric, a_v, b_v, delta, pval_str, better) in enumerate(rows_data):
                for ci, cell in enumerate([metric, a_v, b_v, delta, pval_str]):
                    item = QTableWidgetItem(str(cell))
                    if ci == 3 and better is not None:
                        item.setForeground(QColor(GREEN if better else RED))
                    elif ci == 4:
                        cell_s = str(cell)
                        if "ns" in cell_s or cell_s == "—":
                            item.setForeground(QColor(MUTED))
                        elif "***" in cell_s or "**" in cell_s:
        ...
        def _row(label, a_m, a_s, b_m, b_s, a_runs=None, b_runs=None, lower=True):
            if a_m == 0 and b_m == 0:
                return (label, "—", "—", "—", "—", None)
            d = pct(a_m, b_m)
            ...
            if a_runs and b_runs:
                ...
                p_str = "—"
            return (label, f"{a_m:.1f} ± {a_s:.1f}", f"{b_m:.1f} ± {b_s:.1f}", delta, p_str, better)

        tbl_rows = [
            _row("Emergency latency (ms)",  A["emerg_lat_mean"],  A["emerg_lat_std"],  B["emerg_lat_mean"],  B["emerg_lat_std"],  A.get("runs_emerg_lat"),  B.get("runs_emerg_lat")),
            _row("Medium latency (ms)",     A["med_lat_mean"],    A["med_lat_std"],    B["med_lat_mean"],    B["med_lat_std"],    A.get("runs_med_lat"),    B.get("runs_med_lat")),
            _row("Normal latency (ms)",     A["normal_lat_mean"], A["normal_lat_std"], B["normal_lat_mean"], B["normal_lat_std"], A.get("runs_normal_lat"), B.get("runs_normal_lat")),
            _row("P95 emerg latency (ms)",  A["p95_emerg_mean"],  A["p95_emerg_std"],  B["p95_emerg_mean"],  B["p95_emerg_std"],  A.get("runs_p95_emerg"),  B.get("runs_p95_emerg")),
            _row("P99 emerg latency (ms)",  A["p99_emerg_mean"],  A["p99_emerg_std"],  B["p99_emerg_mean"],  B["p99_emerg_std"],  A.get("runs_p99_emerg"),  B.get("runs_p99_emerg")),
            _row("Throughput (pkts/s)",     A["throughput_mean"], A["throughput_std"], B["throughput_mean"], B["throughput_std"], A.get("runs_throughput"), B.get("runs_throughput"), lower=False),
            _row("Packet loss rate (%)",    A["loss_rate_mean"],  A["loss_rate_std"],  B["loss_rate_mean"],  B["loss_rate_std"],  A.get("runs_loss_rate"),  B.get("runs_loss_rate")),
            _row("PDR (%)",                 A["pdr_mean"],        A["pdr_std"],        B["pdr_mean"],        B["pdr_std"],        A.get("runs_pdr"),        B.get("runs_pdr"),        lower=False),
